...
| 13 | Scale-to-zero cold start can be a 1 GB image pull (15m50s measured) when the pod wakes onto a node with no cached layers — self-reinforcing, since an app at 0 replicas is prime kubelet image-GC bait | Before wiring freecad (larger image, same trap), decide: LAN registry pull-through cache via Talos `registries.mirrors`, node pinning, or accept | [ephemeral-apps-keda.md](ephemeral-apps-keda.md) §"The real constraint" |
